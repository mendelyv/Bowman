 ...
 class Camera
    {
        ...
        public setZero(x:number, y:number):void {
            ...
            var value:number = this._map.width - StageUtils.WIN_WIDTH;
            Camera.zeroX = Camera.zeroX < 0 ? 0 : Camera.zeroX;
            if(this._map.width>StageUtils.WIN_WIDTH) Camera.zeroX = Camera.zeroX > value ? value : Camera.zeroX;

            value = this._map.height - StageUtils.WIN_HEIGHT;
            Camera.zeroY = Camera.zeroY < 0 ? 0 : Camera.zeroY;
            if(this._map.height>StageUtils.WIN_HEIGHT) Camera.zeroY = Camera.zeroY > value ? value : Camera.zeroY;

        }

        public update():void {
            if (this._focus) this.setZero(this._focus.posX - (StageUtils.WIN_WIDTH >> 1),this._focus.posY - (StageUtils.WIN_HEIGHT >> 1));

            Camera._cameraView.x = Camera.zeroX;
            Camera._cameraView.y = Camera.zeroY;

            Camera._cameraView.width = StageUtils.WIN_WIDTH;
            Camera._cameraView.height = StageUtils.WIN_HEIGHT;
        }
        ...
        public get cameraCutView():egret.Rectangle {
            ...
            this._cameraCutView.width = StageUtils.WIN_WIDTH + this._map.tileWidth * 2;
            this._cameraCutView.height = StageUtils.WIN_HEIGHT + this._map.tileHeight * 2;
        ...
        public lookAt(x:number, y:number):void {
            this.focus = null;
            this.setZero(x - (StageUtils.WIN_WIDTH >> 1), y - (StageUtils.WIN_HEIGHT >> 1));
            this.reCut();
        }
        ...
        public flyTo(x:number, y:number, callback:Function = null):void {
            ...
            this._moveStart = new egret.Point(Camera.zeroX - (StageUtils.WIN_WIDTH >> 1), Camera.zeroY - (StageUtils.WIN_HEIGHT >> 1));
            this._moveEnd = new egret.Point(x - (StageUtils.WIN_WIDTH >> 1), y - (StageUtils.WIN_HEIGHT >> 1));
