...
class BaseMap implements IMap {
     ...
     * 路点宽度
     */
    private _roadW: number = 48;
    /**
     * 路点高度
     */
    private _roadH: number = 48;
     ...
     * @param blockh 区块高度
     */
    public createLoop(id: number, bg: string, callback: Function, thisobj: any, blockw: number = 48, blockh: number = 48): void {
        var that: BaseMap = this;
        let map: egret.Bitmap = Util.createBitmap(bg);
        that._mapid = id;
        that._tileW = 40;//map.texture.textureWidth;
        that._tileH = 40;//map.texture.textureHeight;
    ...
    //             StageUtils.WIN_WIDTH = that._dbuffer.stage.stageWidth;
    //             StageUtils.WIN_HEIGHT = that._dbuffer.stage.stageHeight;
    //         }, this);
    //     } else {
    //         StageUtils.WIN_WIDTH = this._dbuffer.stage.stageWidth;
    //         StageUtils.WIN_HEIGHT = this._dbuffer.stage.stageHeight;
    //     }
    // }
     /**
         * 设置主容器
    ...
    public resize(): void {
        this._areaX = Math.ceil(StageUtils.WIN_WIDTH / this._tileW) + 1;
        this._areaY = Math.ceil(StageUtils.WIN_HEIGHT / this._tileH) + 1;
    ...
    public getPointAround(center: egret.Point, from: egret.Point, r: number): egret.Point {
        ...
        var angle: number = this.getPointAngle(center.x - from.x, center.y - from.y) + (Math.random() > .5 ? 1 : -1) * Math.PI / 8;
        while (i < max) {
     ...
     * 返回为弧度值
     */
    public getPointAngle(x: number, y: number): number {
