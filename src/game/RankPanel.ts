...
class RankPanel extends egret.DisplayObjectContainer 
{
    ...
    public initView(){
        ...
        let enemy = Main.instance.gameView.battleMgr.enemys;
        let exp:number = 100;
        for(let i = 0; i<enemy.length;i++){
            let rankItem: RankItem = new RankItem(this,exp,list_w,i,enemy[i].nickName);
...
class RankItem extends egret.DisplayObjectContainer{
    ...
    public constructor(rankPanel: RankPanel, Exp:number,item_w:number,flag:number,name:string ){
        super();
        this._rankPanel = rankPanel;
        this._itemW = item_w;
        this.width = this._itemW;
        this.height = this._itemH;
        this.drawSelf(item_w,flag,name);
    }

    private drawSelf(item_w:number,flag:number,name:string){
      ...
      this._comNickNameTxt = Util.createText(name,this._comRankTxt.x + 38,this._itemH - 38,30,Util.s_colors.black,false,egret.HorizontalAlign.LEFT); 
