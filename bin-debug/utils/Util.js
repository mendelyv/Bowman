...
var Util = (function () {
     ...
     */
    Util.createBitmap = function (name) {
     ...
     * @constructor
     */
    Util.createBitmap9 = function (filename, rect) {
        if (rect === void 0) { rect = null; }
        var bitmap = Util.createBitmap(filename);
        if (!bitmap)
            return null;
        if (rect) {
     ...
     * @constructor
     */
    Util.setAnchor = function (obj, x, y) {
        if (x === void 0) { x = 0.5; }
        if (y === void 0) { y = null; }
        if (y == null) {
    ...
    //// number数组转字符串;
    Util.arrToString = function (arr, jionStr) {
        var str = '';
        var num = arr.length;
        for (var i = 0; i < num; i++) {
            if (i == 0) {
    ...
    //}
    //// 字符串转数字数组
    Util.strToArrNum = function (strData, jionStr) {
        if (strData == "" || strData == null)
            return [];
        var arr = [];
        var arr2 = Util.strToArrStr(strData, jionStr);
        var num = arr2.length;
        for (var i = 0; i < num; i++) {
     ...
     */
    Util.strToArrStr = function (strData, jionStr) {
        var arr = strData.split(jionStr);
        return arr;
    };
    /***
     * 根据num关键字创建一个Bitmap
     * ***/
    Util.createNum = function (resJson, num) {
        var res = RES.getRes(resJson);
     ...
     */
    Util.parseTxtUrl = function (title) {
        var starIdx = title.indexOf("url=");
        var fontEnd = title.indexOf(">", starIdx);
        var str = title.substring(starIdx + 4, fontEnd).trim();
     ...
     */
    Util.parseTxtImg = function (title) {
        ...
        while (firstIdx < length) {
            var starIdx = title.indexOf("<img", firstIdx);
            //var starIdx:number = title.indexOf("<img src=", firstIdx);
            if (starIdx < 0) {
                firstIdx = length;
            }
            else {
                var fontEnd = title.indexOf(".", starIdx + 36);
                var url = "";
                if (fontEnd != -1) {
                    var extend = title.substring(fontEnd, fontEnd + 4); //扩展名
     ...
     */
    Util.replaceColor = function (oldStr, newStr, con) {
        var firstIdx = 0; //文本段开始位置
        var length = con.length;
        var tempStr = con;
        while (firstIdx < length) {
            var starIdx = tempStr.indexOf(oldStr, firstIdx);
            if (starIdx < 0) {
                firstIdx = length;
            }
            else {
     ...
     */
    Util.createStageMask = function (color, alpha, stageW, stageH) {
        if (color === void 0) { color = 0; }
        if (alpha === void 0) { alpha = 0.5; }
        if (stageW === void 0) { stageW = 480; }
        if (stageH === void 0) { stageH = 800; }
    ...
    //    return null;
    //}
    Util.splitStrToArray = function (str, keyChar) {
        var vt = []; //怪物基础数据
        var count = 0;
        var end = false;
        var pos = 0;
     ...
     * @returns {any}
     */
    Util.getWordBySign = function (sign) {
     ...
     * @returns {number}
     */
    Util.getRandomRound = function (max) {
        var rand = Math.random() * max;
     ...
     * @returns {number}
     */
    Util.getRandomRange = function (min, max) {
        var c = max - min + 1;
     ...
     */
    Util.randomArrayOrder = function (targetArray) {
    ...
    //}
    //判断是否碰撞
    Util.isHit = function (aimrect, hitrect, aimSeffX, hitSeffX) {
        if (aimSeffX === void 0) { aimSeffX = 0; }
        if (hitSeffX === void 0) { hitSeffX = 0; }
    ...
    Util.drawRectangle = function (x, y, width, height) {
     ...
     * @returns {egret.Shape}
     */
    Util.drawLineRectangle = function (x, y, width, height, color, lineW) {
        if (lineW === void 0) { lineW = 1; }
        var shp = new egret.Shape();
     ...
     * @returns {egret.Shape}
     */
    Util.drawFillR = function (width, height, color, lineColor, lineWidth, alpha) {
        if (color === void 0) { color = 0; }
     ...
     * @returns {egret.Shape}
     */
    Util.drawLineRoundRect = function (x, y, width, height, color, lineW, ellipseWidth, ellipseHeight) {
        if (lineW === void 0) { lineW = 1; }
        if (ellipseWidth === void 0) { ellipseWidth = 10; }
        if (ellipseHeight === void 0) { ellipseHeight = 10; }
        var shp = new egret.Shape();
        shp.graphics.lineStyle(lineW, color);
    ...
    // }
    /**判断点是否在矩形框内 */
    Util.isInRect = function (x, y, rect) {
        var left = rect.x;
        var top = rect.y;
     ...
     * @return
     */
    Util.fontOpenTagByColor = function (color) {
     ...
     */
    Util.createText = function (text, x, y, size, color, bold, align) {
        if (size === void 0) { size = 22; }
        if (color === void 0) { color = 0; }
        if (bold === void 0) { bold = false; }
        if (align === void 0) { align = egret.HorizontalAlign.CENTER; }
        var font = new egret.TextField();
        font.text = text;
        font.x = x;
        font.y = y;
        font.size = size;
     ...
     */
    Util.createMC = function (dataName, imgName, name, frameRate, loops, fun, funParent) {
        if (loops === void 0) { loops = 0; }
        if (fun === void 0) { fun = null; }
        if (funParent === void 0) { funParent = null; }
        var data = new egret.MovieClipDataFactory(RES.getRes(dataName), RES.getRes(imgName));
        var mc = new egret.MovieClip(data.generateMovieClipData(name));
     ...
     * @param value
     */
    Util.saveDataToLocal = function (key, value) {
     ...
     * @returns {string}
     */
    Util.getDataToLocal = function (key) {
     ...
     * @param key
     */
    Util.removeDataToLocal = function (key) {
        egret.localStorage.removeItem(key);
    };
    /**
     * 清空本地浏览器所有存储的数据
     */
    Util.clearDataToLocal = function () {
